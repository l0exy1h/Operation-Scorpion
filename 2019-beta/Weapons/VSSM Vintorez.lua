...
function weaponData.getModel(pp)
	if pp == "tpp" then
		...
		local ret = clone(script.vssmvintorez_t)
		ret.Name = "FppGun"
		return ret
	end
end
...
function weaponData.getStats()
	return {
		...
		recoilXRec         = 9.35,
		recoilYMult        = 1,
		recoilYPattern     = "3,2,2;6,0",
		recoilYRan         = 0.12,
		recoilYRec         = 2.9,
		recoilYStart       = 2,
		recoilZ            = 0.38,
		recoilZBackDur     = 0.1,
		recoilZReturnDur   = 0.33,
		rps                = 11.88,
		spread             = 13,
		weight             = 0.998,
		supportedFireModes = {
...
local function getFppPoses()
	return {
		...
		holding = {
			-- FppRoot = newCf(0.05, 0.15, 0.2),
			FppLeftArm = newCf(-0.7016, -0.3453, -1.5495, 0.8139, 0.1806, -0.5523, -0.4664, 0.7700, -0.4354, 0.3466, 0.6119, 0.7109),
			FppRightArm = newCf(0.4604, 0.3803, 0.2296, 1.0000, 0.0000, 0.0000, 0.0000, 1.0000, 0.0000, 0.0000, 0.0000, 1.0000),
		},
		sprinting = {
			FppLeftArm = newCf(-0.8351, -0.8078, -0.8450, 0.9686, -0.2487, -0.0063, 0.2243, 0.8838, -0.4106, 0.1077, 0.3963, 0.9118),
	...
	return {
		...
		tppAnimations = {
			holding = {
				[1] = {
					goalC0 = {
						LeftUpperArm = newCf(-0.56801939, -0.0317719057, -0.376664788, 0.993709564, 0.103784837, 0.0420708507, 0.0030035926, 0.35083884, -0.936430991, -0.111947432, 0.930666804, 0.348320186),
		...
		aniparts = {
			WeaponMain = "primary",
			WeaponMag  = "vssmvintorez_stdmag.vssmvintorez_stdmag",
			WeaponBolt = "f.vssmvintorez_bolt",
			ShellP0    = "f.ShellP0",
			ShellD0    = "f.ShellD0",
			FirePoint  = "f.Firepoint",
			AimPart    = "f.aimpart"
		},
		invisibleAniparts = {
			FirePoint = 1,
		...
		sounds = sounds,
		shell = script["9x39_casing"],
	}
end

function weaponData.getDefaultAttachments()
	return {
		Magazine = "vssmvintorez_stdmag",
		Stock    = "vssmvintorez_stdstock",
	}
end
