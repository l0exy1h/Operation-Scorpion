...
function weaponData.getModel(pp)
	if pp == "tpp" then
		...
		local ret = clone(script.krissvector_t)
		ret.Name = "FppGun"
		return ret
	end
end
...
function weaponData.getStats()
	return {
		...
		recoilXRec         = 9.7,
		recoilYMult        = 3.2,
		recoilYPattern     = "2,0.2",
		recoilYRan         = 0.7,
		recoilYRec         = 2.5,
		recoilYStart       = 1,
		recoilZ            = 0.2,
		recoilZBackDur     = 0.1,
		recoilZReturnDur   = 0.22,
		rps                = 20,
		spread             = 9,
		weight             = 0.975,
		supportedFireModes = {
...
local function getFppPoses()
	return {
		...
		holding = {
			FppLeftArm = newCf(-0.7282, -0.4998, -1.5060, 0.8139, 0.1806, -0.5523, -0.4664, 0.7700, -0.4354, 0.3466, 0.6119, 0.7109),
			FppRightArm = newCf(0.4567, 0.3879, -0.0047, 1.0000, 0.0000, 0.0000, 0.0000, 1.0000, 0.0000, 0.0000, 0.0000, 1.0000),
		},
		sprinting = {
			FppLeftArm = newCf(-0.7374, -1.0286, -0.7288, 0.9686, -0.2487, -0.0063, 0.2243, 0.8838, -0.4106, 0.1077, 0.3963, 0.9118),
	...
	return {
		...
		tppAnimations = {
			holding = {
				[1] = {
					goalC0 = {
						LeftUpperArm = newCf(-0.521674693, -0.0881768167, -0.232215673, 0.993709564, 0.0981866941, 0.0538575761, 0.00300373835, 0.457381964, -0.889265239, -0.111947507, 0.883833051, 0.454209924),
						LeftLowerArm = newCf(-4.76837158e-07, -0.200000331, 1.04308128e-07, 0.999997735, 0.00211213715, -0.000192055188, -0.00192230195, 0.864400446, -0.502800167, -0.000895970268, 0.502799392, 0.864402533),
		...
		aniparts = {
			WeaponMain = "primary",
			WeaponMag  = "krissvector_stdmagazine.krissvector_stdmagazine",
			WeaponBolt = "f.krissvector_charginghandle",
			ShellP0    = "f.ShellP0",
			ShellD0    = "f.ShellD0",
		...
		sounds = sounds,
		shell = script["45acp_casing"],
	}
end

function weaponData.getDefaultAttachments()
	return {
		Magazine = "krissvector_stdmagazine",
		Muzzle   = "krissvector_stdmuzzle",
		Stock    = "moecarbinestock",
		Optic    = "troybattlesights",
		Barrel   = "krissvector_stdhandguard",
	}
end
