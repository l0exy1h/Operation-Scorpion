...
function weaponData.getModel(pp)
	if pp == "tpp" then
		...
		local ret = clone(script.mp5a3_t)
		ret.Name = "FppGun"
		return ret
	end
end
...
function weaponData.getStats()
	return {
		...
		recoilXDampDur     = 13,
		recoilXDampExp     = 1,
		recoilXDampInit    = 0.95,
		recoilXRec         = 7.8,
		recoilYMult        = 1.35,
		recoilYPattern     = "3,1,2",
		recoilYRan         = 0.44,
		recoilYRec         = 3,
		recoilYStart       = 2,
		recoilZ            = 0.24,
		recoilZBackDur     = 0.1,
		recoilZReturnDur   = 0.245,
		rps                = 13.3,
		spread             = 10.85,
		weight             = 0.97,
		supportedFireModes = {
...
local function getFppPoses()
	return {
		...
		holding = {
			FppRightArm = newCf(0.49080193, 0.134035319, -0.118456125, 1, 0, 0, 0, 1, 0, 0, 0, 1),
			FppLeftArm = newCf(-0.730839372, -0.45342496, -1.51781893, 0.813855469, 0.180640742, -0.552275538, -0.466363013, 0.770028412, -0.435386986, 0.346619219, 0.611902952, 0.71093595),
		},
		sprinting = {
			FppLeftArm = newCf(-0.8010, -0.8207, -0.5805, 0.9968, -0.0802, -0.0063, 0.0705, 0.9091, -0.4106, 0.0386, 0.4089, 0.9118),
			FppRightArm = newCf(1.5544, -0.0229, -0.7954, 0.5285, -0.1118, 0.8415, 0.0655, 0.9937, 0.0910, -0.8464, 0.0070, 0.5325),
		}
	}
end
function weaponData.getAniData()
	...
	return {
		...
		aniparts = {
			WeaponMain = "primary",
			WeaponMag  = "mp5a3_stdmagazine.mp5a3_stdmagazine",
			WeaponBolt = "f.mp5a3_bolt",
			ShellP0    = "f.ShellP0",
			ShellD0    = "f.ShellD0",
...
function weaponData.getDefaultAttachments()
	return {
		Magazine = "mp5a3_stdmagazine",
		Stock    = "mp5a3_stdstock",
		Barrel   = "mp5a3_stdhandguard",
	}
end
