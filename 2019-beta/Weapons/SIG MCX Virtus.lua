...
function weaponData.getModel(pp)
	if pp == "tpp" then
		...
		local ret = clone(script.mcxvirtus_t)
		ret.Name = "FppGun"
		return ret
	end
end
...
function weaponData.getStats()
	return {
		...
		recoilXDampDur     = 11,
		recoilXDampExp     = 1,
		recoilXDampInit    = 0.73,
		recoilXRec         = 8.5,
		recoilYMult        = 1.15,
		recoilYPattern     = "3,-0.9,5;7,0.65;14,0.88,3;20,0.1",
		recoilYRan         = 0.22,
		recoilYRec         = 0.98,
		recoilYStart       = 3,
		recoilZ            = 0.31,
		recoilZBackDur     = 0.1,
		recoilZReturnDur   = 0.28,
		rps                = 10.2,
		spread             = 11,
		weight             = 1.002,
		supportedFireModes = {
...
local function getFppPoses()
	return {
		...
		holding = {
			FppLeftArm = newCf(-0.7934, -0.3509, -1.2806, 0.8139, 0.1806, -0.5523, -0.4664, 0.7700, -0.4354, 0.3466, 0.6119, 0.7109),
			FppRightArm = newCf(0.4477, 0.2691, 0.1456, 1.0000, 0.0000, 0.0000, 0.0000, 1.0000, 0.0000, 0.0000, 0.0000, 1.0000),
		},
		sprinting = {
...
function weaponData.getAniData()
	...
	return {
		...
		aniparts = {
			WeaponMain = "primary",
			WeaponMag  = "stanagmag.stanagmag",
			WeaponBolt = "f.mcxvirtus_bolt",
			ShellP0    = "f.ShellP0",
			ShellD0    = "f.ShellD0",
...
function weaponData.getDefaultAttachments()
	return {
		Magazine = "stanagmag",
		Handle   = "mcxvirtus_stdgrip",
		Muzzle   = "mcxvirtus_stdmuzzle",
		Stock    = "mcxvirtus_stdstockext",
		Optic    = "troybattlesights",
		Barrel   = "mcxvirtus_stdhandguard",
	}
end
