-- 180803 playeranimation before character rewrite
...
while true do
	for name, _aplr in pairs(aplrs) do
		if _aplr.customChar and _aplr.customChar:FindFirstChild("Torso") then
			...
			warn(string.format("aplr %s doesnt have a customchar or torso %s %s", name, tostring(_aplr.customChar), tostring(_aplr.customChar.Torso)))
		end
	end
	wait(1)
end