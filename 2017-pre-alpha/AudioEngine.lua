...
function event_Event(func,args)
	if args[1] == nil then
		warn("sound to be played is nil") 
		return 
	end
	if func == "Play" then
...
function event_PlaySound(s,cnew)
	...
	if s:FindFirstChild("SoundStats") == nil then
		warn("play1")
		if string.find(s.Name, "Water/") then 
			s.Volume = 10
			warn("play2") 
		end
		s:play()
...
function ml_UpdateAudio(dat,s,inst,connected)
	...
	--[[if string.find(s.Name, "Water") then
		warn(SoundStats.Volume.Value, SoundStats.VolumeExponent.Value, s.Volume)
	end--]]
