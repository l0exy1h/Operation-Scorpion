	...
	if C0.p.magnitude > 10	
		...
		if secureC0verbose then
			if tick() - secureC0st > secureC0errGap then
				...
				warn("\n-------------------bug-------------------")
				warn(string.format("[bug report] lp = %s, weld = %s, c0ax = %s, c0ay = %s, magnitude = %s, C0 = %s", lp.Name, weld:GetFullName(), tostring(C0aX), tostring(C0aY), tostring(weld.C0.p.magnitude), tostring(C0)))
				warn("-------------------bug-------------------\n")
			end
		end
		return false
...
local function attc_Item(item, customChar, cm)
	...
	if customChar and customChar:FindFirstChild("Torso") then
		...
		error("torso motor6d missing for %s when attaching", customChar.Name, item)
	end
end
...
function AnimatedPlayer.new(plr)
	...
	self.customChar = self.plr.Character
	...
	if self.isLocal then
		...
		Instance.new("BodyPosition", char.Torso) 
	end
...
function AnimatedPlayer:rekt()
	...
	if not self.deathHandled then
		...
		fakeTorso.Name = self.name.."Clothes"
		fakeTorso.Transparency = 1
		fakeTorso.Parent = workspace.Ragdolls
		fakeTorso.Size = customChar.Torso.Size
		fakeTorso.CFrame = customChar.Torso.CFrame
		
		-- weld every clothes to a new part		
		for _, v in ipairs(customChar.Torso:GetChildren()) do
			if v:IsA("Weld") then
				if v.Part1:IsDescendantOf(workspace.Clothes) and v.Part0:IsDescendantOf(workspace.Alive) then
					v.Parent = fakeTorso
					v.Part0  = fakeTorso
				end 
			end
		...
		-- move every piece of clothes out of workspace.clothes
		local cm = self:getClothesModel()
		if cm then
			for _, piece in ipairs(cm:GetChildren()) do
				piece.Parent = fakeTorso
			end
			cm:Destroy()
		end
		fakeTorso.Anchored = false
		fakeTorso.CanCollide = true
...
function AnimatedPlayer:handleShooting(dist, weapon)
	...
	if weapon:FindFirstChild("Fire") and weapon:FindFirstChild("Cam_Aim") then
		...
		if hit then
			...
			-- only register the hit in characters (body parts), i.e. everything in workspace.Alive is hitboxes.
			-- ignore clothing and etc (see the ignore list)
			if hit:IsDescendantOf(workspace.Alive) and self.isLocal and hit.Parent.Parent == workspace.Alive then
				...
				mainRemote:FireServer("changeHealth", {hit.Parent.Name, -dmg, gearName, Ray.new(Vector3.new(1, 2, 3), Vector3.new(4, 5, 6))})
...
function AnimatedPlayer:renderUpdate()
	...
	local headC0 = characterWelds.Head[3] * ttilt * CFrame.fromEulerAnglesYXZ(math.rad(client.angleX) * (1-client.torsoLook)-tiltfwd+math.rad(client.freeX * client.freeP), math.rad(client.freeY * client.freeP), leanamt+headtilt)
	customTorso.Head.C0 = secureC0("Head", headC0, customTorso.Head)
...
function AnimatedPlayer:main()
	if self.isLocal then  		-- for the local player
		spawn(function()
			...
			print(string.format("render and camera thread ended for local player %s because of death detected", self.name))
		end)
		spawn(function()
			while self.isAlive do
				hb:wait()
			...
			print(string.format("camera fx thread ended for local player %s because of death detected", self.name))
		end)
		...
		spawn(function()
			...
			print(string.format("all thread ended for non local player %s because of death detected", self.name))
		end)	
	end
